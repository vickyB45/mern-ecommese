...
import { WEBSITE_CART, WEBSITE_HOME, WEBSITE_SHOP } from "@/routes/WebsiteRoute";

import { Star, Truck, RefreshCcw, ShieldCheck, ShoppingCart } from "lucide-react";
...
const ProductDetails = ({ product, variant, colors, sizes, reviewCount, similarProducts }) => {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Product Section */}
      <div className="lg:flex lg:items-start lg:gap-10 lg:px-20 px-4">
        {/* Left: Images */}
            ...
            {thumbnails.map((thumb, idx) => (
              <div
                ...
                className={`cursor-pointer border rounded-md overflow-hidden ${
                  activeImage === thumb ? "border-red-500" : "border-transparent"
                }`}
              >
                <Image
                  src={thumb}
        ...
        <div className="lg:w-1/2">
          ...
          <p className="text-gray-600 mb-6">
            {product?.shortDescription || "Premium quality product crafted with attention to detail."}
          </p>
          ...
          <div className="flex justify-around items-start gap-6 mb-6">
            <div>
              <h3 className="md:text-xl font-semibold mb-2 text-gray-800">Available Colors</h3>
              <div className="flex gap-3 flex-wrap">
                {(colors?.length ? colors : product?.colors || ["No Color Available"]).map((color, idx) => (
                  <span key={idx} className="px-3 py-1 border rounded-full text-sm text-gray-700 bg-gray-100">
                    {color}
                  </span>
                ))}
              </div>
            ...
            <div>
              <h3 className="md:text-xl font-semibold mb-2 text-gray-800">Available Sizes</h3>
              <div className="flex gap-3 flex-wrap">
                {(sizes?.length ? sizes : product?.sizes || ["No Size Available"]).map((size, idx) => (
                  <span key={idx} className="px-3 py-1 border rounded-full text-sm text-gray-700 bg-gray-100">
                    {size}
                  </span>
                ))}
      ...
      <div className="lg:px-20 px-4 mt-16 pb-20">
        ...
        <p className="text-gray-500">
          {reviewCount > 0 ? `${reviewCount} reviews available.` : "No reviews yet."}
        </p>
      </div>

      {/* Similar Products */}
      <div className="max-w-6xl mx-auto">
        <div className="text-3xl md:text-[45px] font-bold py-6">Similar Products</div>
