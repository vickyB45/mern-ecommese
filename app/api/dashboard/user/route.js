...
export async function GET(request) {
  ...
  try {
    ...
    // Get recent orders
    const resentOrder = await orderModel
    ...
    const totlOrder = await orderModel.countDocuments({ user: userId });

    return response(true, 200, "Dashboard Info", { resentOrder, totlOrder });
  } catch (error) {
